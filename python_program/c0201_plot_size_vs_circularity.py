...
def plot_size_vs_circularity():
    ...
    for record_name in record_name_master:
        ...
        plt.scatter(x = np.linspace(a, a, 100), y = np.linspace(0.9*min(circularity), 1.1*max(circularity), 100), s = 1, c = [.8,0,0], label = "Min Area")
        # plt.scatter(x = np.linspace(a_2, a_2, 100), y = np.linspace(0.9*min(circularity), 1.1*max(circularity), 100), s = 1, c = [0,0,.8])
        plt.scatter(x = np.linspace(0.9*min(area), 1.1*max(area), 100000), y = np.linspace(.4, .4, 100000), s = 1, c = [0,0,.8], label = "Min Circularity")

        plt.scatter(area, circularity, c = [.8,.8,.8], label = "Non-Colonies")

        area_truncated, circularity_truncated = truncate_two_lists(area, circularity, a, a_2, 0.4, 10)
        plt.scatter(area_truncated, circularity_truncated, c = [1,0,0], label = "Colonies")
        ...
        area_truncated, circularity_truncated = truncate_two_lists(area, circularity, 0, a, 0.4, 10)
        plt.bar([0], [len(area_truncated)], width=0.8)

        area_truncated, circularity_truncated = truncate_two_lists(area, circularity, 0, a, 0, .4)
        plt.bar([2], [len(area_truncated)], width=0.8)

        area_truncated, circularity_truncated = truncate_two_lists(area, circularity, a, a_2, 0, .4)
